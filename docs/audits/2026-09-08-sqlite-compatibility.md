...
### Completed full Rust regression at production 9360743

The existing all-target root 25173 terminated with exit 0; it was not restarted while observations were pending. The command was cargo test --all-targets --no-fail-fast -j 2 -- --test-threads=1. All 58 Cargo groups have final successful summaries: 685 passed, zero failed, ten explicitly ignored. The library group passed 253 with six ignored in 339.11 seconds, after the 46.16-second build. The final summary was regenerated after terminal exit and checked for missing or failed groups, counting only each containing suite's last summary to avoid nested-child duplication.

The four integration ignores are the two separately exercised native migration/old-writer fixtures recorded above and two explicit performance fixture generators. Debug all-target results are not release latency evidence, and this command does not run Doc-tests. This refresh supersedes the 211cae3 full regression for current production, including native inventory file opens, exact-bound query configuration reuse and the shared-facts capture reduction. It does not close formal performance, freshness, resource, complete host lifecycle or retirement gates. Evidence: target/verification/task6-9360743-full-rust.log and task6-9360743-full-rust-summary.json, with root exit verified independently of log parsing.

## Current c5c2097 frontend regression refresh

The current branch passed all 199 JavaScript tests in tests/dock_renderer.cjs and tests/metro_core.cjs (zero failures, cancellations or skips; 1971.3481 ms). All five files named in the frozen runtime-baseline.json match their recorded SHA-256 exactly: assets/dock.html, assets/metro-core.js, src/dock_asset.rs, tests/dock_renderer.cjs and tests/dock_ui_contract.rs. This verifies the branch files against the frozen installed-overlay baseline; it is not a new inspection or modification of the currently installed plugin.

Evidence: target/verification/task6-c5c2097-javascript.log and task6-c5c2097-asset-hashes.json. The current complete Rust run is still active under root 50640 and has no final result yet. These JavaScript/source checks do not replace current native browser rendering, real host lifecycle, performance or P5 acceptance.

## Current c5c2097 native migration, browser pair and late old writer

A new disposable legacy-process-TkobAM fixture was generated through the hash-verified frozen A1CFBB1C executable. The current all-target compilation produced migration_native_fixture-0aebb95b1cac3cbc.exe SHA-256 78B2A52B4A7004A6A9874140C8A0615F5B14EB1B6984C99857330BCAD710843E. Its explicit export_native_migration_pair test exited 0 and passed in 3.40 seconds: migrated SQL and frozen legacy full models match, and the saved real old-process model matches after normalizing only two process-local transport counters. Fixture creation and export logs are task6-c5c2097-native-{create,export}.log.

The real Chromium comparison of this backend snapshot pair exited 0 (root 31189). Across widths 1280, 560 and 360, the 24 baseline-repeat/candidate comparisons for initial/details/zoom-pan/refresh had zero visible differing pixels, zero excluded pixels and maximum channel delta 2 within the fixed tolerance. Interaction states matched; the deliberate pixel mutation failed comparison as required. Clock/build metadata were fixed and animations disabled. Assembled resource SHA-256 remains 1F2BF0BEA4844BF6E6E59A57AF3DD7DC1C62507F0F7E66F8C87EBFE007405C36. The 560-pixel candidate workspace-details image was visually inspected. Evidence: task6-c5c2097-native-browser/report.json and its PNGs, plus task6-c5c2097-native-browser.log. This is actual rendering of current migrated backend snapshots; it is not simultaneous live backend lifecycle, actual host integration or performance evidence.

The same current fixture executable subsequently passed native_old_writer_is_detected_without_losing_either_store in 0.86 seconds with exit 0. The frozen old binary appended a legacy event after migration, verification diagnosed drift, and both SQL state and the frozen snapshot were preserved. Evidence: task6-c5c2097-native-late-writer.log and legacy-process-TkobAM/late-writer-report.json. TkobAM is now deliberately divergent; do not repair, reimport or reuse it as a clean migration fixture. The full Rust run under root 50640 remains active, so this entry does not claim complete regression acceptance. These separate native/browser checks overlapped that run; no latency comparison is inferred.

## Retained db2abdf actual owner restart refresh

The retained production CLI was verified as SHA-256 13D8B43BEDE032A44FD8827916EC57059A45021B70B2116EF87E1C03D4F12395 before copying it into a fresh owned shared-browser-0b5OT5 fixture. The actual MCP/HTTP/SSE/Chromium restart harness exited 0 (root 34313). MCP PID 18472 remained stable while owner 19536 was replaced by owner 24428 with a different instance identity. The same browser URL, seven workspaces, six tasks, expanded details, 140% zoom, scroll (26,70) and keyboard focus were preserved. The after.png screenshot was visually inspected.

The selected aria-current marker changed from one to none, matching the frozen frontend refresh behavior already established separately. This report does not claim selection retention or pixel parity across restart. The replacement owner was observed running from the exact owned fixture executable with --idle-seconds 60; no unknown process was terminated. Evidence: target/verification/task6-db2abdf-owner-restart.log and shared-browser-0b5OT5/report.json plus screenshots. The harness ran while full Rust regression root 50640 was active; it establishes reconnect behavior, not latency or idle-resource acceptance. Production remains db2abdf, and the full goal remains open.

## Retained db2abdf actual Codex CLI MCP refresh

The retained production executable was hash-verified as 13D8B43BEDE032A44FD8827916EC57059A45021B70B2116EF87E1C03D4F12395 and exercised through the currently available Codex CLI 0.154.0-alpha.6.2 at the bffc5354119c8421 host path. The ephemeral, ignore-user-config session operated only on new fixture codex-host-Qe2Px1. Root 23295 exited 0 without expiration. Its event stream contains exactly three successful devmap_candidate MCP calls: read_map, set_route_plan, read_map. The accepted route and final readback match the original repository/worktree identity, goal and revision 1.

A separate replay assessment passed and a deliberately incorrect readback failed the verifier as required. Evidence: task6-db2abdf-codex-host.log, task6-db2abdf-codex-host-recheck.log and codex-host-Qe2Px1/{manifest.json,events.jsonl,report.json,report-validated.json}. A subsequent process inspection found no devmap.exe running from the exact retained candidate path. No installed plugin or global configuration was changed. This establishes an actual CLI MCP smoke, not automatic hook delivery, desktop navigation, full host lifecycle or resource/performance acceptance. Full Rust root 50640 remained active during this run; overall goal acceptance remains open.

## Current complete Rust regression

Root 50640 exited 0 for cargo test --all-targets --no-fail-fast -j 2 -- --test-threads=1 on source c5c2097 (retained production db2abdf). The final summary contains 58 Cargo groups, 685 passed, zero failed and ten explicitly ignored entries, with no missing group result. The library group passed 253 tests and ignored six fixture/diagnostic entrypoints in 324.99 seconds; compilation took 44.41 seconds. The other ignores are two explicit native migration/late-writer fixtures, separately passed above on this build, and two opt-in performance fixture generators. The summarizer uses the last result per Cargo group so nested fixture output is not double-counted. No doc-test or debug-performance claim is added.

Evidence: target/verification/task6-c5c2097-full-rust.log and task6-c5c2097-full-rust-summary.json, plus the retained terminal exit code. cargo fmt --check passed after the run; the source-identical c5c2097 all-target Clippy run with denied warnings was already passed in 8.81 seconds. Later commits through this entry only update audit prose. Frontend JS, frozen hashes, native migration/browser, owner restart and real Codex CLI scopes are recorded above; this full regression does not close latency, freshness, complete host lifecycle, platform or P5 gates.

## 9f3a5bf native migration and browser refresh

Fresh frozen-A1CF fixture legacy-process-1pUAxj exported through current debug fixture SHA-256 21815000D7E8B5530ADD0A12409FFF45DF9512B4A977D530F5269E48396895C9. Export passed once in 3.05 seconds; original revision and observation_revision are the only normalized transport counters. Browser root 30880 exited 0 with 24 comparisons at widths 1280/560/360 and four interaction states: zero visible differing pixels, zero excluded pixels, maximum channel delta 2 within allowance 2, negative control passed. Resource hash remained 1F2BF0BEA4844BF6E6E59A57AF3DD7DC1C62507F0F7E66F8C87EBFE007405C36. The 560-pixel candidate details image was inspected.

Explicit late-old-writer test passed in 0.87 seconds, preserving SQL and the old writer's new bytes. Fixture 1pUAxj is now deliberately divergent and must not be repaired or reused as clean. Logs: task6-9f3a5bf-native-{create,export,browser,late-writer}.log, native-exe-sha.txt and native-browser/report.json. These tests ran during full Rust regression and do not measure performance.

## Owner hard-termination cleanup failure found on current candidate

The current actual CLI 50DAC5C8 passed functional restart assertions in shared-browser-2RGLIw: MCP PID 27500 survived owner 8044 to 28780, with the same URL, seven workspaces, six tasks, expanded state, 140% zoom and scroll 26/70. Focus remained on map-tools-trigger; aria-current changed from one marker to none, matching the known frozen refresh behavior, not proving selection retention. The after image was inspected.

However, harness root 7267 did not exit after its functional report. MCP 27500 and replacement owner 28780 subsequently disappeared, but Node 32824 remained alive. Inspection identified Git PID 32388, parent 8044, created at 2026-09-13 09:36:35.984538 +01:00, with the exact owned fixture command: git -C .../shared-browser-2RGLIw/repository show-ref --verify --quiet refs/heads/dev. Its only thread 18692 was waiting with ThreadWaitReason.Suspended. No direct children of Node were present. A separate process sharing an old parent PID was unrelated and was not touched.

After rechecking the exact Git PID/name/parent/command, only Git 32388 was stopped. It and Node 32824 then disappeared, and root 7267 finally exited 0. This required manual cleanup and therefore FAILS unattended owner-termination cleanup, regardless of the harness's functional passed flag. Identity evidence is retained in task6-9f3a5bf-orphan-git.json.

Source inspection confirms a vulnerable interval: git_process::supervise spawns the suspended child before Tree::attach creates/assigns the kill-on-close Job and resumes it. The observed suspended orphan is consistent with owner death in that interval; deterministic reproduction is still needed. Prioritize atomic process-to-Job assignment at creation (or an equivalently proven lifecycle design), with a regression that kills the owner in this interval and proves child handles signal. Do not weaken checks, merely close inherited pipes to mask survival, or classify manual termination as acceptance. Existing owner restart evidence remains functional-only until fixed and reverified. Full Rust root 65220 remains live; the complete goal is not achieved.

## Dedicated owner lifetime Job closes the pre-attach gap

A Windows owner now installs a private, non-inheritable kill-on-close Job before creating its reactor or identity helper. The owner joins this Job before any child is created; Windows automatically associates descendants during process creation. Existing per-command Jobs remain nested below it. No breakaway is enabled. A single process-lifetime handle is retained until OS teardown, because closing it while its member owner is alive would terminate that owner. Repeated installation is idempotent; setup errors stop owner startup before spawning children. Clients and their independently launched owners are not placed in a shared new lifetime Job.

This uses documented [Job inheritance and nested kill-on-close behavior](https://learn.microsoft.com/en-us/windows/win32/procthread/job-objects). It avoids replacing Rust/Tokio argument quoting and pipe creation. The protection is installed at the dedicated owner boundary; this change does not assert a new hard-kill guarantee for arbitrary embedded library callers outside that owner.

Release test root 62301 exited 0 after a 94-second build: two tests passed in 0.23 seconds. The deterministic test spawns a suspended child, retains its native process handle, then kills the fixture owner before per-command attach/resume. Unprotected control stays alive as expected and is explicitly reaped by its retained handle. Protected and nested-Job cases signal without manual child termination. A separate protected normal-exit fixture passed in 20.02 seconds (root 8724, exit 0), and its child was absent afterward. All-target Clippy passed in 13.65 seconds; actual CLI build completed in 70 seconds (root 83821, exit 0).

Actual candidate SHA-256 95906C5C994DC36415599F2DF33336A87F1CD72C5F076F2C8A7831DD88E483FB is retained in task6-candidate-owner-lifetime/devmap.exe. Real restart root 96965 exited 0 automatically, without manual cleanup, using shared-browser-UWqMyP. MCP 13864 retained the browser while owner 27180 was replaced by 31068. Seven workspaces, six tasks, expanded state, 140% zoom, scroll 26/70 and keyboard focus remained; aria-current still loses the marker as in the frozen frontend. The after image was inspected. No fixture Git processes remained. Replacement owner 31068 was first verified at the exact fixture executable with --idle-seconds 60, and subsequently observed absent without a termination request. No exit-code/resource claim is inferred for that detached replacement.

This resolves the reproduced dedicated-owner crash gap and the observed restart-harness hang. Broader performance/full-host/platform/P5 gates remain open. The full regression still running in root 65220 was compiled before this change and proves only source 9f3a5bf when complete. Evidence: task6-owner-lifetime-{tests,normal,clippy,cli-build,restart}.log, normal-root.txt, cli-sha.txt, and shared-browser-UWqMyP/report.json.

### Full pre-lifetime regression completed; current candidate refresh underway

Root 65220 exited 0. The 9f3a5bf all-target Rust log has 58 complete successful Cargo groups: 687 passed, zero failed and 10 ignored. The summarizer selects the final result in each group to avoid counting nested subprocess test output twice. The library group passed 255 with six ignored in 333.13 seconds. Evidence: target/verification/task6-9f3a5bf-full-rust.log and full-rust-summary.json. This source predates the owner lifetime fix and is not full-regression evidence for febbb69.

Current actual CLI 95906C5C (febbb69) smoke root 21417 exited 0 automatically, run shared-summary-1fJPyo. Cold observations were 4004.684 and 3896.691 ms; four clients with three measured hot requests each gave p95 221.570, 223.232, 222.318 and 223.189 ms. Maximum result size was 8781 bytes. SQL and immutable preservation passed, read-cohort preservation was true, and errors/cleanup errors were empty. All nine owned children have terminal harness-stop records. Single-change detection/restoration took 3931.828/3503.428 ms and the fixture was restored; these are correctness smoke observations, not formal freshness acceptance. Both latency gates remain false. The sample is too small and not a controlled comparison to attribute the apparent hot-query difference to the Job fix.

After the smoke root terminated, current febbb69 formatting passed and all-target Rust root 24933 started. It is still running and no completed candidate regression is claimed. Logs: target/verification/task6-febbb69-fmt.log, full-rust.log, cdrive-smoke.log and cdrive-smoke-config.json. Formal performance, resource, full host/platform and remaining P0-P5 gates remain open.

### Actual host refresh for febbb69

Current actual CLI SHA-256 95906C5C994DC36415599F2DF33336A87F1CD72C5F076F2C8A7831DD88E483FB passed the ephemeral Codex CLI MCP fixture codex-host-OxiNB3. Codex version was freshly checked as 0.154.0-alpha.6.2 at the bffc5354119c8421 executable path. Root 46725 exited 0 without expiry. The event stream contains exactly read_map, set_route_plan, read_map; all calls succeeded, repository/worktree identity matched and the accepted route goal and revision 1 were present on readback. The separate validator rejected a modified readback goal and saved report-validated.json. Temporary configuration used ignore-user-config; no installed plugin or global configuration was modified.

This is real CLI MCP evidence, not automatic hook, desktop navigation, cross-platform or performance acceptance. It overlapped the current full Rust regression, root 24933, which is still live. Evidence: target/verification/task6-febbb69-codex-host.log and codex-host-OxiNB3/manifest.json, events.jsonl, report.json and report-validated.json. Do not rerun the write-once validator against this completed fixture.

### Complete febbb69 regression exposed a handshake-fixture race

Root 24933 terminated with exit 101. All 58 Cargo groups have final results: 688 passed, one failed, 10 ignored. The failing case was abandoned_route_response_keeps_structured_cas_and_old_request_result_after_later_update in shared_runtime_mutations; send failed at line 202 with Windows error 232 (BrokenPipe), rather than a route-state assertion. Every other group passed. The unchanged failing case passed alone with backtraces enabled in 10.42 seconds (root 27635, exit 0). That rerun does not erase the full-run failure.

The fixture prepared its Hello after opening the pipe: hashing the executable and running Git synchronously consumed the owner's existing two-second Hello deadline. Preparing the same complete Hello before opening removes this avoidable scheduling exposure, matching the intended ready-to-handshake client behavior. No production timeout, authentication field, route/retry assertion or server behavior changed. The original failure lacked a call-site backtrace, so this is an identified fixture defect and plausible explanation, not a proven exclusive root cause.

After root 24933 terminated, the corrected complete seven-test shared_runtime_mutations group passed in 48.66 seconds after an 11.42-second build (root 3646, exit 0), using default parallel test execution. Formatting and diff checks passed. This is focused correction evidence; do not relabel the failed whole-run log as successful. Logs: target/verification/task6-febbb69-full-rust.log, full-rust-summary.json, route-failure-recheck.log and task6-hello-preparation-tests.log.

### Current complete regression and staged acceptance amendment

Root 42729 exited 0 on production 4e426e8. All 58 Cargo groups have successful final summaries: 690 passed, zero failed, 10 ignored. This includes the workspace-inspection overlap and the corrected preconnection handshake fixture. The old febbb69 failure remains recorded; it is not overwritten by this new successful run. Evidence: target/verification/task6-4e426e8-full-rust.log and full-rust-summary.json. Subsequent commits through the two-stage plan and simplification ledger change documentation only.

User-authorized delivery is now governed by docs/superpowers/plans/2026-09-13-devmap-two-stage-acceptance.md. Stage 1 retains correctness, compatibility, real host/user-flow, cleanup, resources and actual simplification hard gates, with preregistered old/new comparative performance. The 3-second/200-ms/2-second absolute objectives remain Stage 2. Neither this full regression nor Stage 1 alone proves every original objective achieved. The new baseline-only calibration and formal comparison have not yet run.
