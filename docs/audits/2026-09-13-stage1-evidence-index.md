...
新增[当前 CLI 操作演练](2026-09-13-storage-operator-rehearsal.md)：4SjQ71 新样例迁移、重复迁移、SQL 新写入与幂等重试、一致性备份/防覆盖、旧版迟到写入拒绝及两份历史保持通过。操作指南已列出实际决策边界；不代表已实现分歧历史自动合并、任意恢复或真实用户受控试用。

[旧版负载诊断](../../tests/browser/stage1-host-load-diagnostic.md) prkP0X 完成 1 冷/四客户端各 20 次短测并保持数据；测试期间机器平均忙碌约 76%、可用内存最低约 503 MiB，测试结束后的 30 秒仍约 33% 忙碌。只说明当前环境存在持续负载，不归因于具体进程或分页。继续保留失败 A/A、等待可测量的较安静窗口；不冻结正式合同、不扩大容忍度，不用短测代替校准。

[SessionEnd 超时修复](2026-09-13-hook-shutdown-budget.md)：4C91 原生命令三次均超出宿主默认 1 秒。5dd8288 显式设置官方支持的 3 秒，23 个安装/契约检查通过，新 release 实际安装与配置复验通过。更改定义需宿主重新审核；实际自动触发、冷/高负载关闭及记录落库仍待验证，未声称此项完全关闭。

同一审计新增 OHStk1：25BF 在确认空闲 owner 端点消失后，原生 SessionEnd 约 1.69 秒完成；开始/结束 SQL 记录身份、顺序和哈希关联正确，storage verify 通过。仅覆盖单次无强杀冷结束，不等同真实宿主审核/触发或高负载截止时间验收。已请求隔离仓库信任审核例外，未收到授权前保持信任记录不变。
...
新增[公共全量地图变更测量预检](../../tests/browser/full-map-change-preflight.md)：oeTHlP 冻结旧版四客户端完成两次预热、四次测量变更，观测版本与地图版本分别验证，恢复后完整模型/旧数据一致，代理 EOF 正常退出、严格 Job 清空。仅补齐原生公共路径的小样例测量工具；100 次规模变更 A/A、浏览器反馈校准和正式比较仍未完成。RE3E2E 的新 30 秒 OS 观察仍约 33% 忙碌，没有据此重复旧版全量 A/A。

同一预检后续 LMi2Na 对每次收敛的脏/净完整模型验证精确字段变化，24 个客户端/变更结果均通过；4 项验证器控制通过。LOFrrS 故意中断第二次测量，准确保留 1 完成、1 失败、2 未执行，探针和旧数据恢复、四代理正常退出、失败 Job 清空。补齐小工具的模型语义及中断统计，不能关闭 100 次规模校准或生产故障恢复门槛。

新增[带受控探针的规模输入与预检](../../tests/browser/legacy-change-scale.md)：CSuFvK 注册新 h7vZyC 数据集（20 worktree / 100 会话 / 10 万记录），探针先于记录生成，逐条 HEAD 校验通过。Hs40L8 四客户端规模预检通过，32 份完整响应的全部工作区/会话归属正确，旧数据与原 jI7MaE 清单保持，代理正常退出、严格 Job 清空。旧版变更观察 5.9–6.4 秒仅为预检；100 次 A/A 和其他校准仍未关闭，不能混用两个数据集的统计。

变更 A/A [运行与分析协议](../../tests/browser/stage1-change-calibration.md)已在 b247576 提交：A1/A2 各 10 次预热、100 次测量、四客户端，按 10 次变更配对区块分析；7 项控制通过。jvYk1v 已按该协议启动，两组终态及分析尚未核实。它仅使用旧 A1CF，不是正式候选人口，也没有冻结或放宽容忍度；以运行目录中的终态报告为后续判定依据。

jvYk1v 后续终态已核实：controller 退出 0，两组共 200 次变更 / 800 客户端结果完整，数据保持、八个代理正常退出、两个严格 Job 清空。896 份响应的工作区/会话归属复核通过。但冻结分析器判定五个 A/A 区间均超出 ±250 ms，校准不通过；旧版第二组变快不能用作候选性能证据。运行后独立 OS 窗口约 4.5% 忙碌，较运行前约 32% 安静，已登记在同协议下开展一个独立新批次的依据；原失败人口完整保留。正式合同仍未冻结。

## 公共端点工具预检（不用于延迟验收）

第二次变更 A/A（2Q4Xwp）已结束：800 个测量结果完整，模型、旧数据、输入和退出检查通过，但五个噪声区间仍超出 ±250 ms，校准再次不通过。两次失败人口均保留，未冻结正式合同。后续有界诊断发现旧版 Git 启动链产生大量进程：QSxoL3 完整采样峰值 147 个进程，其中 137 个 Git 启动器/原生进程；SiXuhS 仅修改测试子进程 PATH 后峰值 24，模型和旧数据一致。该结果只说明环境路径值得调查，不是新版收益、噪声已解决或正式环境变更。具体结果与限制见[校准记录](../../tests/browser/stage1-change-calibration.md)。实际可信自动 hooks 的授权门槛仍未关闭。
