...
- Continuous history with at least four ordinary interior commits folds into two retained endpoints, a three-dot rail break and an expandable hash/subject/count summary. Forks, merges, workspace HEADs, refs/tags, explicit boundaries and journey anchors stay visible. Expansion survives observations and advancing HEADs; parent/child navigation uses the original edges and reveals hidden targets. The original snapshot is unchanged.
- The self-contained resource regression budget is 184 KiB, including the searchable workspace chooser, persistent inspector controls, arrival navigation and reversible history summaries; no external UI dependencies are added. The 512 KiB runtime resource cap and snapshot validation limits remain unchanged.
...
- **Primary selection:** Clicking a v4 workspace opens one inline summary beside its HEAD label. Shared HEADs first offer exact workspace choices. The expanded card shows copyable Branch, full HEAD and Worktree values, full workspace facts, recorded destinations and task details without a second workspace-details action. Labels stack above values so narrow cards preserve readable text width. Repeat click or Escape collapses it. Selecting another workspace resets summary scroll; refreshing the same one preserves scroll and focus, with fallback when a task disappears.
- **Geometry:** Chat cards and expanded workspace details grow with measured content height. The map owns their scrolling; neither region has an independent scrollbar. Geometry reserves the measured height, including text wrapping and expanded task content. Label collision placement and association stems can move; actual commit coordinates and historical rails remain unchanged. Selection preserves the workspace screen anchor and map zoom. Unavailable or partial destination observations remain explicit and never establish a parent branch.
- **Container:** One floating details panel has sticky title/actions and a scrolling content area. Commit details anchor synchronously beside the selected node with a connecting line, without scrolling an already visible node. Narrow layouts use the available space above or below the node to avoid covering it. Exact values wrap within the panel; the 44px hit target remains larger than the visible station.
