...
fn bundled_skill_has_a_narrow_honest_trigger() {
    ...
    assert!(skill.contains("stable task-ID tie-break"));
    assert!(skill.contains("take at most 64"));
    assert!(skill.contains("omit history"));
    assert!(skill.contains("roster—including live tasks—is partial"));
