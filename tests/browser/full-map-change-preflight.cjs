...
async function main(){
 ...
 async function read(client,label){
  ...
  if(client.previousCycle!==null)assert.equal(model.observation_revision,client.previousCycle+1,'Frozen old per-client refresh counter');
  client.previousCycle=model.observation_revision;
  const parsed=performance.now();return {model,parsed,bytes:packet.wire_bytes,packet,label};
 }
 ...
 try{
  ...
  for(let trial=0;trial<6;trial++){
   const row={index:trial,phase:trial<2?'warmup':'measured',dirty:trial%2===0,status:'running',clients:clients.map((c,i)=>({client:i,prior:c.previousCycle,polls:[],status:'pending'}))};report.trials.push(row);
   ...
   try{
    for(let round=0;row.clients.some(c=>c.status==='pending');round++){
     ...
     outcomes.forEach((outcome,i)=>{const c=active[i];if(outcome.status==='rejected'){c.status='error';c.error=String(outcome.reason.stack||outcome.reason);failed=true;return;}
      ...
      if(data.valid&&data.validated<deadline){c.status='converged';c.elapsed_ms=data.validated-start;}else if(data.validated>=deadline){c.status='timeout';failed=true;}
     });
    ...
    row.status='complete';
   }catch(e){row.status='failed';row.error=String(e.stack||e);throw e;}finally{save();}
  }
  ...
  report.children=ctx.children.map(c=>({pid:c.child.pid,exit:c.exit}));report.completed=report.errors.length===0&&report.trials.length===6&&report.trials.every(t=>t.status==='complete');save();
 }
